...
const BulkStatus = forwardRef((props, ref) => {
	...
	// Function to create a file from base64 data
	const createFileFromBase64 = (dataURI: string) => {
		try {
			// Extract the base64 data
			const b64Data = dataURI.split(",")[1] || dataURI;
			...
			// Create blob with explicit type that matches allowed extensions
			const blob = new Blob([ab], { type: "text/plain" });

			// Create file with .txt extension (one of the allowed extensions)
			return new File([blob], "emails.txt", { type: "text/plain" });
		} catch (error) {
			console.error("Error creating file from base64:", error);
			throw new Error("Failed to create file from base64 data");
		}
	};

	// Try API direct call with data URL
	const performApiDirectCall = () => {
		logDebug("Performing direct API call with data URL");

		// Use the exact format from the example
		const form = new FormData();
		form.append(
			"file",
			"data:text/plain;name=testEmailsList.txt;base64,dXNoYS5rdW1hcmlkQGZmZi5jb20KTmVoYS5iaGFydGlAZGZnLmNvbQplcm1lZ2lsaXVzQGdtYWlsLmNvbQp0ZXN0dXNlcjFAZXhhbXBsZS5jb20gIAp0ZXN0dXNlcjJAZXhhbXBsZS5jb20gIAp0ZXN0dXNlcjNAZXhhbXBsZS5jb20gIAp0ZXN0dXNlcjRAZXhhbXBsZS5jb20gIAp0ZXN0dXNlcjVAZXhhbXBsZS5jb20="
		);
		...
		fetch(`/api/upload-bulk-emails?api_key=${API_KEY}`, options)
			.then((res) => res.text())
			.then((text) => {
				logDebug("Direct API call response text:", text);
				try {
					...
					setUploadResponse({ raw: text, parseError: true });
				}
			})
			.catch((err) => {
				console.error("Direct API call error:", err);
				setUploadResponse({ error: String(err) });
			});
	};
